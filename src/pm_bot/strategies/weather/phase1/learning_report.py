"""Cross-run learning summaries for weather final scorecards."""
...
def build_weather_learning_report(
    *,
    ...
    monotonicity_gap_total = 0.0
    monotonicity_gap_count = 0
    ...
    for scorecard_path in scorecard_paths:
        ...
        monotonicity_gap = payload.get("average_monotonicity_gap_bps")
        if isinstance(monotonicity_gap, (int, float)):
            monotonicity_gap_total += float(monotonicity_gap)
            monotonicity_gap_count += 1
    ...
    average_monotonicity_gap_bps = (
        round(monotonicity_gap_total / monotonicity_gap_count, 4)
        if monotonicity_gap_count > 0
        else 0.0
    )
    ...
    return WeatherLearningReport(
        ...
        average_monotonicity_gap_bps=average_monotonicity_gap_bps,
        average_actionable_market_ratio=average_actionable_market_ratio,
    )


def format_weather_learning_report(report: WeatherLearningReport) -> str:
    lines = [
        ...
        f"- average_monotonicity_gap_bps: {report.average_monotonicity_gap_bps:.4f}",
...
def write_weather_learning_report(
    *,
    report: WeatherLearningReport,
    output_path: str | Path,
) -> None:
    target_path = Path(output_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(format_weather_learning_report(report), encoding="utf-8")
...
def _infer_focus(payload: dict[str, Any]) -> str:
    ...
    settlement_audit_status = str(payload.get("settlement_audit_status", "")).strip()
    if tradable_subset_status != "ready":
        return "selection"
    if settlement_audit_status != "ready":
        return "settlement"
    return "execution"
...
def _recommended_actions(focus: str) -> tuple[str, ...]:
    if focus == "selection":
        return (
            "tighten weather market selection around stable threshold series",
            "remove recurring thin or blocked forecast runs from the active set",
        )
    if focus == "settlement":
        return (
            "review settlement mapping and strip monotonicity for repeated fragile runs",
            "re-check station and skill inputs for the weakest weather families",
        )
    if focus == "execution":
        return (
            "keep current weather working preset and collect more run windows",
        )
    return ("collect more weather scorecard runs before changing presets",)
