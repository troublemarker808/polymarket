"""Controlled rollback for weather preset applications."""
...
class WeatherPresetRollbackResult:
    rolled_back: bool
...
def rollback_weather_preset_application(
    *,
    ...
    if blockers:
        return WeatherPresetRollbackResult(
            rolled_back=False,
...
def format_weather_preset_rollback_result(result: WeatherPresetRollbackResult) -> str:
    lines = [
        "# Weather Preset Rollback Result",
        "",
...
def write_weather_preset_rollback_result(
    *,
    result: WeatherPresetRollbackResult,
    output_path: str | Path,
) -> None:
    target_path = Path(output_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(
        format_weather_preset_rollback_result(result),
        encoding="utf-8",
    )
...
def _load_restored_preset_names(path: Path) -> tuple[str, ...]:
    ...
    for section in ("threshold", "ensemble"):
