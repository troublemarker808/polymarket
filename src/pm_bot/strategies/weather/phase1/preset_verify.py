"""Verification helpers for applied weather preset changes."""
...
class WeatherPresetVerificationReport:
...
def build_weather_preset_verification_report(
    *,
    baseline_scorecard_path: str | Path,
    candidate_scorecard_path: str | Path,
) -> WeatherPresetVerificationReport:
    ...
    if candidate_action != "proceed":
        reasons.append("candidate weather scorecard no longer remains proceed-grade")
    if readiness_delta < -0.03:
        reasons.append("candidate weather readiness_score regressed materially versus baseline")
    if total_profit_loss_delta > 0.2:
        reasons.append("candidate weather total_profit_loss increased materially versus baseline")

    if reasons:
        if candidate_action == "pause" or len(reasons) >= 2:
            verification_decision = "rollback"
            next_step = "rollback to the previous weather working preset and collect another replay window"
        else:
            verification_decision = "hold"
            next_step = "keep the weather candidate preset under review and collect another verification scorecard"
    else:
        verification_decision = "pass"
        next_step = "keep the weather candidate preset as the next working preset and continue observation"

    return WeatherPresetVerificationReport(
        ...
        reasons=tuple(reasons) if reasons else ("candidate weather verification remains stronger than or equal to baseline",),
        next_step=next_step,
    )


def format_weather_preset_verification_report(report: WeatherPresetVerificationReport) -> str:
    lines = [
        "# Weather Preset Verification Report",
        "",
...
def write_weather_preset_verification_report(
    *,
    report: WeatherPresetVerificationReport,
    output_path: str | Path,
) -> None:
    target_path = Path(output_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(
        format_weather_preset_verification_report(report),
        encoding="utf-8",
    )
