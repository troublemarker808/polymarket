"""Lifecycle helpers for weather preset promotion."""
...
from pm_bot.strategies.weather.phase1.tuning_plan import build_weather_candidate_preset_registry


@dataclass(slots=True, frozen=True)
class WeatherPresetLifecycle:
    working_preset: str
...
def build_weather_preset_lifecycle(
    *,
    report: Any,
    ...
    candidate_registry = build_weather_candidate_preset_registry(plan=report.tuning_plan)
...
def format_weather_preset_lifecycle(lifecycle: WeatherPresetLifecycle) -> str:
    lines = [
...
def build_weather_working_preset_patch(
    *,
    ...
    if lifecycle.promotion_decision != "promote_candidate" or not lifecycle.apply_ready:
        return {}
    candidate_registry = build_weather_candidate_preset_registry(
...
def write_weather_preset_lifecycle(
    *,
    lifecycle: WeatherPresetLifecycle,
    output_dir: str | Path,
) -> None:
    ...
    (target_dir / "preset_lifecycle.md").write_text(
        format_weather_preset_lifecycle(lifecycle) + "\n",
        encoding="utf-8",
    )
...
def write_weather_working_preset_patch(
    *,
    lifecycle: WeatherPresetLifecycle,
    report: Any,
    output_dir: str | Path,
