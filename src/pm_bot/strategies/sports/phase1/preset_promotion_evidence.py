"""Evidence aggregation for sports preset promotion decisions."""
...
class SportsPresetPromotionEvidence:
    report_count: int
    winning_variant: str
    ready_to_apply: bool
...
def build_sports_preset_promotion_evidence(
    *,
    ...
    if recurring_promotion_count < 2:
        blockers.append("same sports candidate has not won enough repeated auto-experiments")
    if recurring_targeted_improvement < 0.03:
        blockers.append("targeted sports loss improvement is not strong enough across repeated auto-experiments")

    return SportsPresetPromotionEvidence(
...
def format_sports_preset_promotion_evidence(evidence: SportsPresetPromotionEvidence) -> str:
    lines = [
...
def write_sports_preset_promotion_evidence(
    *,
    ...
    target_path.write_text(
        format_sports_preset_promotion_evidence(evidence),
        encoding="utf-8",
    )
