"""Bundled change package for sports working preset updates."""
...
class SportsPresetChangePackage:
    ...
    evidence: SportsPresetPromotionEvidence
    patch: dict[str, dict[str, object]]


def build_sports_preset_change_package(
    *,
    lifecycle: SportsPresetLifecycle,
    evidence: SportsPresetPromotionEvidence,
    report: Any,
    base_match: dict[str, str] | None = None,
) -> SportsPresetChangePackage:
    if hasattr(report, "baseline") and hasattr(report, "winner"):
        version_comparison = build_sports_version_comparison(report=report, evidence=evidence)
        ...
        strategy_state_reason = (
            "sports promotion evidence is ready for a candidate preset"
            if evidence.ready_to_apply
            else "sports preset still needs more evidence"
        )
    ...
    return SportsPresetChangePackage(
...
def format_sports_preset_change_package(package: SportsPresetChangePackage) -> str:
    lines = [
...
def write_sports_preset_change_package(
    *,
    ...
    target_path.write_text(
        format_sports_preset_change_package(package) + "\n",
        encoding="utf-8",
    )
