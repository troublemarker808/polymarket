"""Controlled application of sports preset change packages."""
...
def apply_sports_preset_change_package(
    *,
    ...
    if not isinstance(decoded, dict):
        raise ValueError("sports preset change package must be a JSON object")
    ...
    if not ready_to_apply:
        blockers.append("sports change package is not ready_to_apply")
    if not isinstance(patch, dict) or not patch:
        blockers.append("sports change package patch is empty")
    ...
    if blockers:
        return SportsPresetApplyResult(
    ...
    anchor_registry = _resolve_registry(config, ("strategy", "anchor", "preset_registry"))
    live_registry = _resolve_registry(config, ("strategy", "live", "preset_registry"))
    for preset_name, preset_payload in patch.items():
        if isinstance(preset_name, str) and isinstance(preset_payload, dict):
            anchor_registry[preset_name] = preset_payload
            live_registry[preset_name] = preset_payload
...
def format_sports_preset_apply_result(result: SportsPresetApplyResult) -> str:
    lines = [
        "# Sports Preset Apply Result",
        "",
...
def write_sports_preset_apply_result(
    *,
    result: SportsPresetApplyResult,
    output_path: str | Path,
) -> None:
    target_path = Path(output_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(
        format_sports_preset_apply_result(result),
        encoding="utf-8",
    )
...
def _build_verification_steps(*, next_working_preset: str, output_path: Path, backup_path: Path) -> tuple[str, ...]:
    return (
        ...
        f"run a fresh sports replay using {output_path.name} and confirm {next_working_preset} still produces proceed-grade results",
        "compare readiness_score and total_profit_loss against the previous sports working preset window",
        "only promote the applied config into a broader sports profile after the verification replay remains stable",
    )
...
def _build_rollback_steps(*, backup_path: Path, output_path: Path, rollback_output_path: Path) -> tuple[str, ...]:
    return (
        ...
        "re-run the previous sports working preset verification replay before re-attempting promotion",
    )
