"""Controlled application of crypto phase2 preset change packages."""
...
class CryptoPhase2PresetApplyResult:
    ready_to_apply: bool
    applied: bool
...
def apply_crypto_phase2_preset_change_package(
    *,
    ...
    if not isinstance(decoded, dict):
        raise ValueError("preset change package must be a JSON object")
    ...
    if not ready_to_apply:
        blockers.append("change package is not ready_to_apply")
    if not isinstance(patch, dict) or not patch:
        blockers.append("change package patch is empty")
    ...
    if blockers:
        return CryptoPhase2PresetApplyResult(
    ...
    phase2 = _resolve_phase2_config(config)
    preset_registry = phase2.setdefault("preset_registry", {})
    if not isinstance(preset_registry, dict):
        preset_registry = {}
        phase2["preset_registry"] = preset_registry

    for preset_name, preset_payload in patch.items():
        if isinstance(preset_name, str) and isinstance(preset_payload, dict):
            preset_registry[preset_name] = preset_payload
...
def format_crypto_phase2_preset_apply_result(result: CryptoPhase2PresetApplyResult) -> str:
    lines = [
        "# Crypto Phase 2 Preset Apply Result",
        "",
...
def write_crypto_phase2_preset_apply_result(
    *,
    result: CryptoPhase2PresetApplyResult,
    output_path: str | Path,
) -> None:
    target_path = Path(output_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(
        format_crypto_phase2_preset_apply_result(result),
        encoding="utf-8",
    )
...
def _resolve_phase2_config(document: dict[str, Any]) -> dict[str, Any]:
    strategy = document.setdefault("strategy", {})
    if not isinstance(strategy, dict):
        strategy = {}
        document["strategy"] = strategy
    phase2 = strategy.setdefault("phase2", {})
...
def _build_verification_steps(
    *,
    ...
    return (
        ...
        f"run a fresh crypto phase2 suite using {output_path.name} and confirm {next_working_preset} still produces proceed-grade results",
        "compare readiness_score, total_profit_loss, and filtered_pnl against the previous working preset window",
        "only promote the applied config into a live profile after the verification suite remains stable",
    )
...
def _build_rollback_steps(
    *,
    ...
    return (
        ...
        "re-run the previous working preset verification suite before re-attempting promotion",
    )
